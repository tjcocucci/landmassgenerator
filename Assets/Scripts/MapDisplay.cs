...
public class MapDisplay : MonoBehaviour
{
    ...
    public void DisplayMeshColorMap(float[,] noiseMap, ColorLevel[] colorLevels, float heightMultiplier, AnimationCurve animationCurve) {
        int width = noiseMap.GetLength(0);
        int height = noiseMap.GetLength(1);
        Color[] colorMap = TextureGenerator.GenerateLeveledColorMap(noiseMap, colorLevels);
        Texture texture = TextureGenerator.TextureFromColorMap (colorMap, width, height);
        MeshData meshData = MeshGenerator.GenerateMeshData(noiseMap, heightMultiplier, animationCurve);
